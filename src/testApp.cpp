...
void testApp::update() {
    ...
    trackerPosMapped.x = ofGetWindowWidth() - trackerPosMapped.x; // mirror the cam X screen
...
void testApp::draw() {
    ...
    if (gamestate == 0) {
        ...
        ofPushStyle();{
            ofSetColor(0);
            fonttitle.drawStringCentered("FASTFOOD", ofGetWindowWidth()/2, 200);
            font22.drawStringCentered("YOU HAVE ONE MINUTE TO EAT AS MUCH AS YOU CAN!", ofGetWindowWidth()/2, 300);
        }ofPopStyle();
        ...
        ofPushStyle();{
            ...
            ofSetColor(255, 20);
            ofRect(ofGetWindowWidth()/2, ofGetWindowHeight()/2, 250, 40);
        }ofPopStyle();
        font16.drawStringCentered(username, ofGetWindowWidth()/2, ofGetWindowHeight()/2);
        
        //PRESS TO START
        ofSetColor(ofRandom(255),0,0,80);
        font30.drawStringCentered("PRESS 'RETURN' WHEN YOU ARE READY", ofGetWindowWidth()/2, ofGetWindowHeight()/2 + 300);
        ...
        ofPushStyle();{
            ...
            font30.drawStringCentered("YOUR SCORE IS "+ofToString(score), ofGetWindowWidth()/2, 150);
            font22.drawStringCentered("HIGH SCORES", ofGetWindowWidth()/2, 250);
        }ofPopStyle();
        ...
        ofPushStyle();{
            ...
            if (savedScores.size() < 8) {
                for (int i = 0; i < savedScores.size(); i++) {
                    font16.drawString(ofToString(i+1) + ". ", 420, 330 + i * 28); // ROW
                    font16.drawString(sortedNames[i], 450, 330 + i * 28); // NAME
                    font16.drawString(ofToString(sortedScores[i]), 970, 330 + i * 28); // SCORE
                }
        ...
        ofColor color;
        color.set(251, ofRandom(255), 64);
        ofSetColor( color );
        font30.drawStringCentered("PRESS 'SPACEBAR' TO RESTART", ofGetWindowWidth()/2, ofGetWindowHeight()/2 + 200);
        
    }
    
    ofDrawBitmapString(ofToString(ofGetFrameRate()), 30, ofGetWindowHeight()-30);
...
void testApp::mousePressed(int x, int y, int button){
    ofPushStyle();{
        ofRectangle _switch = ofRectangle(ofGetWindowWidth()/2-150, ofGetWindowHeight()/2-50, 300, 100);
